...
function normalPath(ctx, points, spanGaps, vm) {
  ...
  var lineMethod = steppedLine ? require$$0.canvas._steppedLineTo : require$$0.canvas._bezierCurveTo;
  var move = true;
...
function (_Element) {
  ...
  _createClass(Point, [{
    ...
    value: function draw(ctx, chartArea) {
      ...
      if (chartArea === undefined || require$$0.canvas._isPointInArea(vm, chartArea)) {
        ctx.strokeStyle = vm.borderColor;
        ctx.lineWidth = vm.borderWidth;
        ctx.fillStyle = vm.backgroundColor;
        require$$0.canvas.drawPoint(ctx, pointStyle, radius, x, y, rotation);
      }
    }
  }]);
