...
ETO.User.Driver = function() {
    ...
    etoFn.init = function(config) {
        ETO.extendConfig(this, config, 'driver');
    };

    etoFn.getIncomeTooltip = function() {
        var excluded = [];

        if (parseInt(ETO.settings('driver_income.payment_charges', 0)) === 0) {
            excluded.push(ETO.trans('driver_income.payment_charges'));
        }
        if (parseInt(ETO.settings('driver_income.additional_items', 0)) === 0) {
            excluded.push(ETO.trans('driver_income.additional_items'));
        }
        if (parseInt(ETO.settings('driver_income.child_seats', 0)) === 0) {
            excluded.push(ETO.trans('driver_income.child_seats'));
        }
        if (parseInt(ETO.settings('driver_income.discounts', 0)) === 0) {
            excluded.push(ETO.trans('driver_income.discounts'));
        }
        if (parseInt(ETO.settings('driver_income.meet_and_greet', 0)) === 0) {
            excluded.push(ETO.trans('driver_income.meet_and_greet'));
        }
        if (parseInt(ETO.settings('driver_income.parking_charges', 0)) === 0) {
            excluded.push(ETO.trans('driver_income.parking_charges'));
        }

        if(excluded.length > 0) {
            return ETO.trans('booking.charges_excluded') + " " + excluded.join(', ');
        }
    ...
    etoFn.getIncome = function(quote, formId, routeId, userCommision) {
        ...
        if (typeof quote[routeId].driver_income != 'undefined') {
            if (typeof ETO.Form.config.form.values[formId].booking[routeId].fleet != 'undefined'
                && ETO.Form.config.form.values[formId].booking[routeId].fleet[0].commission != ''
            ) {
                subtotal = parseFloat(ETO.Form.config.form.values[formId].booking[routeId].fleet[0].commission);
            } else {
