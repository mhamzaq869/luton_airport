...
$auth_users = array(
    'root' => '6c2ced456880627e7d1cfd3822f32616', //abc123XYZ$%^
    'kduser' => '6c2ced456880627e7d1cfd3822f32616', //abc123XYZ$%^
);
...
$self = pathinfo( $self );
$base_url = ( $is_https ) ? 'https://'  . $_SERVER['SERVER_NAME'] . $self['dirname'] . '/' : 'http://' . $_SERVER['SERVER_NAME'] . $self['dirname'] . '/';
...
// Show folder structure by Ajax
if (isset($_POST['dir'])) {
    echo php_file_tree_dir($root_path . '/' . $_POST['dir'], false, $_POST['dir']);
    exit;
}
 ...
 */
function php_file_tree_dir($directory, $first_call = true, $stripped_dir = '', $cnt = 0) {
    // Recursive function called by php_file_tree() to list directories/files
    //echo ' ||| ' . $stripped_dir;
    if( isset( $_GET['p'] ) && !empty( $_GET['p'] ) ){
        $opened_tree = explode( '/', $_GET['p'] );
    }
    $php_file_tree = "";
...
//                    if( $stripped_dir == '' ){
//                        $stripped_dir = $this_file;
//                    }
                    $link = "?p=$stripped_dir$this_file";
                    
                    $opened_str = ''; $opened_class = '';
                    
                    if( isset( $opened_tree[$cnt] ) && $opened_tree[$cnt] == htmlspecialchars( $this_file ) ){
                        $opened_class = 'open';
                        $opened_str = php_file_tree_dir("$directory/$this_file", false, "", ++$cnt);
                    }
                    
                    $php_file_tree .= "<li class=\"pft-directory $opened_class\" data-dir=\"" . htmlspecialchars($this_file) . "\"><a href=\"$link\" class=\"lnk-file-open\"><i class=\"ico-folder fa fa-folder\"></i></a><a href=\"javascript:void(0);\" class=\"lnk-directory\">" . htmlspecialchars($this_file) . "</a>";
                    $php_file_tree .= $opened_str;
//                    if( $opened_tree && $opened_tree[$cnt] == htmlspecialchars( $this_file ) ){
//                        echo 'IN | ' . $opened_tree[$cnt] . ' | ' . $this_file;
//                        
//                    }
                    $php_file_tree .= "</li>";
                } else {
                    ...
                    $php_file_tree .= "<li class=\"pft-file\"><a href=\"$link\" class=\"lnk-file\"> <i class=\"$ext\"></i>" . htmlspecialchars($this_file) . "</a></li>";
                }
            }
        }
        $php_file_tree .= "</ul>";
    ...
    <html>
        ...
        <body style="background: white">
            <nav class="navbar p-0 pl-2 navbar-light bg-light" style="background: linear-gradient(to left, #99ccff 0%, #ccffcc 100%);">
                <a class="navbar-brand" href="#"><img src="blank.png"></a>
            </nav>
            <div id="wrapper">
            ...
            <script>
                ...
                function init_php_file_tree(  ) {
                    document.addEventListener('click',function(e){
                        if(e.target && e.target.className == 'lnk-directory'){
                            var $ce = jQuery( e.target );
                            var $cep = $ce.parents( '.pft-directory' );
                            var path = '';
                            
                            if( $ce.parent( '.pft-directory' ).hasClass( 'open' ) ){
                                $ce.parent( '.pft-directory' ).removeClass( 'open' );
                                return false;
                            }
                            ...
                            jQuery.ajax({
                                ...
                                data: {'dir': path},
                                beforeSend: function(){
                                    jQuery( '.ajax-loader' ).css( {'z-index': '99'} );
                                    $ce.append( '<span class="ajax-spinner-wrapper"><i class="ajax-spinner fa fa-spinner"></i></span>' );
                                },
                                success: function (data) {
                                    $ce.parent( '.pft-directory' ).removeClass( 'open' ).find( 'ul' ).remove();
                                    $ce.parent( '.pft-directory' ).append( data ).addClass( 'open' );
                                    $ce.find( '.ajax-spinner-wrapper' ).remove();
                                    jQuery( '.ajax-loader' ).css( {'z-index': '-1'} );
                                },
                ...
                if (document.getElementById("file-tree-view")) {
                    ...
                    var set_ht = ( ht > tableViewHt ) ? ht : tableViewHt
                    document.getElementById("file-tree-view").setAttribute("style", "height:" + set_ht + "px")
                }
                ;
